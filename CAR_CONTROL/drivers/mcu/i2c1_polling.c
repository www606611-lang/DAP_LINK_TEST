...
#define I2C1_POLLING_WAIT_SPINS  200000U
#define I2C1_POLLING_RETRY_COUNT      2U
#define I2C1_POLLING_TX_FIFO_BYTES    8U

static i2c1_polling_snapshot_t g_snapshot;

static void i2c1_polling_configure_controller(void);
static i2c1_polling_result_t i2c1_polling_wait_idle(void);
static i2c1_polling_result_t i2c1_polling_wait_not_busy(void);
static void i2c1_polling_start_register_read(
...
void I2C1Polling_Init(void)
{
    ...
    g_snapshot.last_result = I2C1_POLLING_OK;
    g_snapshot.initialized = true;
    i2c1_polling_configure_controller();
}
...
{
    uint8_t retry;
    i2c1_polling_result_t result = I2C1_POLLING_TIMEOUT;

    if ((data == NULL) || (length == 0U) ||
        (length > I2C1_POLLING_TX_FIFO_BYTES) ||
        (address7 > 0x7FU)) {
        g_snapshot.last_result = I2C1_POLLING_BAD_ARGUMENT;
    ...
    for (retry = 0U; retry < I2C1_POLLING_RETRY_COUNT; retry++) {
        result = i2c1_polling_wait_idle();
        if (result != I2C1_POLLING_OK) {
            I2C1Polling_Recover();
            continue;
        }
        ...
        if (DL_I2C_fillControllerTXFIFO(I2C_1_INST,
                (uint8_t *) data, length) != length) {
            g_snapshot.last_result = I2C1_POLLING_FIFO_ERROR;
            return g_snapshot.last_result;
        }

        DL_I2C_startControllerTransfer(I2C_1_INST, address7,
            DL_I2C_CONTROLLER_DIRECTION_TX, length);
        delay_cpu_cycles(16U);
        result = i2c1_polling_wait_not_busy();
        if (result == I2C1_POLLING_OK) {
            g_snapshot.last_result = I2C1_POLLING_OK;
            return I2C1_POLLING_OK;
        }
        I2C1Polling_Recover();
    }
    ...
    uint8_t address7, uint8_t reg, uint8_t *data, uint16_t length)
{
    ...
    for (retry = 0U; retry < I2C1_POLLING_RETRY_COUNT; retry++) {
        ...
        if (DL_I2C_fillControllerTXFIFO(
                I2C_1_INST, &reg, 1U) != 1U) {
            g_snapshot.last_result = I2C1_POLLING_FIFO_ERROR;
            return g_snapshot.last_result;
        }

        i2c1_polling_start_register_read(address7, length);
        delay_cpu_cycles(16U);

        for (index = 0U; index < length; index++) {
            uint32_t spins = I2C1_POLLING_WAIT_SPINS;
            ...
            data[index] = DL_I2C_receiveControllerData(I2C_1_INST);
        }

        result = i2c1_polling_wait_not_busy();
        if ((index == length) && (result == I2C1_POLLING_OK)) {
            g_snapshot.last_result = I2C1_POLLING_OK;
            return I2C1_POLLING_OK;
        }
        if (index != length) {
            result = I2C1_POLLING_TIMEOUT;
        }
        I2C1Polling_Recover();
    }
...
static void i2c1_polling_configure_controller(void)
{
    ...
    DL_I2C_setControllerTXFIFOThreshold(
        I2C_1_INST, DL_I2C_TX_FIFO_LEVEL_EMPTY);
    DL_I2C_setControllerRXFIFOThreshold(
        I2C_1_INST, DL_I2C_RX_FIFO_LEVEL_BYTES_1);
...
static i2c1_polling_result_t i2c1_polling_wait_idle(void)
{
    ...
    while ((DL_I2C_getControllerStatus(I2C_1_INST) &
        ...
        if ((DL_I2C_getControllerStatus(I2C_1_INST) &
                DL_I2C_CONTROLLER_STATUS_ERROR) != 0U) {
            return I2C1_POLLING_BUS_ERROR;
        }
        if (spins-- == 0U) {
            return I2C1_POLLING_TIMEOUT;
        }
    }
    return I2C1_POLLING_OK;
}
