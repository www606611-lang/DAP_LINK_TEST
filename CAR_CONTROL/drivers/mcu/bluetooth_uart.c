...
{
    if (DL_UART_Main_getPendingInterrupt(BLUETOOTH_UART_INST) !=
...
void BluetoothUart_WriteText(const char *text)
{
    ...
    while (*text != '\0') {
        next_head = (uint16_t) ((g_tx_head + 1U) %
            BLUETOOTH_UART_TX_SIZE);
        if (next_head == g_tx_tail) {
            return;
        }
        g_tx_buffer[g_tx_head] = (uint8_t) *text;
        g_tx_head = next_head;
        text++;
    }
}
...
static void bluetooth_uart_transmit_pending(void)
{
    while ((g_tx_tail != g_tx_head) &&
        ...
        DL_UART_Main_transmitData(
            BLUETOOTH_UART_INST, g_tx_buffer[g_tx_tail]);
