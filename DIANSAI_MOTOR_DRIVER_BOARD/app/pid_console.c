...
{
    yaw_angle_control_config_t config;
    yaw_angle_control_state_t state;
    char kp[16];
    char ki[16];
    char kd[16];
    char out[24];
    ...
    pid_console_send_text(line);
    (void) snprintf(line, sizeof(line),
        "yaw state en=%u tgt=%ld cur=%ld err=%ld turn=%ld\r\n",
        state.enabled ? 1U : 0U,
        pid_console_round_to_long(state.target_yaw_deg),
        pid_console_round_to_long(state.current_yaw_deg),
        pid_console_round_to_long(state.error_deg),
        pid_console_round_to_long(state.turn_speed_target_pps));
    pid_console_send_text(line);
