...
## 2026-05-19 当前进展
- 速度环和位置环都已经调稳，蓝牙调参、回读、保存、加载都可用。
- 位置环外层已独立，内层速度环直接继承当前速度环配置，不再单独维护一套速度参数。
- 当前可回退点是 `checkpoint/position-cascade-20260519`，提交号 `d8cb773`。
- 当前入口还是 `DAP_LINK_TEST/cmsis_dsp_empty.c`，按键步进逻辑保留，VOFA 输出已降频，刷屏压力小很多。
- 下一步要把 `YawAngleControl` 正式并进入口主逻辑，并统一初始化顺序，避免外环初始化时把速度环重置掉。
- 当前主要痛点还是右轮正反不对称、最小起转 PWM 偏大、串口日志一多就容易乱。
...
### 当前状态
- 速度环已经调稳，蓝牙改参、回读、保存、加载都能正常用。
- 位置环已经改成串级结构，外层只管位置，内层速度环直接继承当前已经调好的速度环。
- 右轮左右不对称问题已经处理到可用状态，不再是之前那种低速起不来、稍微大一点又冲、再大就复位的循环。
- 当前入口仍是 `DAP_LINK_TEST/cmsis_dsp_empty.c`，主逻辑还是偏位置步进测试，下一步准备把角度环正式并进去。
- 当前可靠回退点是 `checkpoint/position-cascade-20260519`，提交号 `d8cb773`。

### 这次调参最折腾的地方
- 每次改参数都要重新烧录，效率很低，所以后来才补了蓝牙 PID 控制台和 Flash 保存/加载。
- 一开始很多参数看起来像是改了，但实际上没有真正进入运行链路，复位后又回到旧状态，或者回到上一次试出来的状态，特别容易让人误判“代码没生效”。
- 右轮最难调：正反向起转 PWM 明显不对称，低速死区大，前馈关掉后基本不动，前馈一大又会顶得很猛，甚至把 MCU 搞到复位。
- 中间还遇到过“PWM 明明有了，但速度跟不上”“目标看着对，实际速度一直贴着零”“输出已经到上限了还是不跟目标”这些现象，最后确认不只是 PID 的问题，而是右轮电机/驱动链路本身就不对称。
- 串口 VOFA 刷太快会把窗口打爆，所以后来把输出频率降下来了，并且空闲零目标时不再重复刷同样的数据。

### 右轮不对称的处理结果
- 没有简单粗暴地把左右轮硬塞成同一套参数，而是把右轮的方向性最小驱动、前馈参考、输出限幅单独处理。
- 现在右轮已经能在可用 PWM 区间内正常跟随目标，不再出现一动就冲顶、反向大跳、或者低速永远起不来的问题。
- 这部分经验后面不要轻易推翻，因为它是在补机械和驱动的不对称，不是简单重调一组 PID 就能替代的。

### 已落地的代码变化
- `DAP_LINK_TEST/app/pid_console.c`
  - 支持 `pid show / save / load / clear`
  - 支持 speed / position / yaw / line 的参数改写和回读
  - 改参后会同步 LCD，并在部分速度参数变化后同步位置环内层速度配置
- `DAP_LINK_TEST/app/pid_tuning_store.c`
  - 把 speed、position、yaw、line 的参数统一落盘保存
  - 复位后可以重新加载当前保存值
- `DAP_LINK_TEST/PID/encoder_position_control.c`
  - 位置环已经串级化
  - 内层速度环从当前全局 speed 配置同步，不再单独维护旧的速度常量
- `DAP_LINK_TEST/cmsis_dsp_empty.c`
  - 当前主入口保留按键步进位置目标
  - 每次加减 `300 count`
  - VOFA 输出已降频到 `250ms`
...
### 后面要做什么
- 把 `YawAngleControl` 正式并进入口主逻辑。
- 统一初始化顺序，避免某个外环初始化时把已经调好的速度层重置掉。
- 给角度环准备一套清晰的入口方式，后面最好能和位置环、速度环一起切换。

### 给后续接手者的硬约束
- 速度环和位置环已经是项目基座，后面默认不要乱改。
- 调参时一次只动一个量，不要同时改很多项。
- 每次改完先看短串口日志，再看实际电机动作，最后再决定要不要烧录下一版。
- 现在最可靠的回退点就是这个 checkpoint，后面如果新改动把控制链弄乱了，直接回到这里。
